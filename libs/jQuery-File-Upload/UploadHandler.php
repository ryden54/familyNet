...
//             	//mvd($file_path);
//                 $originalExif = get_jpeg_header_data( $file_path );
//                 $input_jpeg = new PelJpeg($file_path);
                /* Retrieve the original Exif data in $jpeg (if any). */
//                 $originalExif = $input_jpeg->getExif();
//                 unset($input_jpeg);

//                 mvd(get_Meta_JPEG( $file_path ));
//                 mvd(@exif_read_data($file_path));
//                 mvd(get_EXIF_JPEG( $file_path ));
//                 die();
...
//                     	$versionPath = $upload_dir.(empty($version)?'':$version.'/').$file->name;

//                     	mvd(count($originalExif));die();

//              		   $output_jpeg = new PelJpeg($versionPath);
...
//                      	put_jpeg_header_data($versionPath, $versionPath, $originalExif);
                        if (!empty($version)) {
                            $file->{$version.'_url'} = $this->get_download_url(
                                $file->name,
                                $version
