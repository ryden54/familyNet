...
$context->addCssFile('\static\libs\gallery\css\blueimp-gallery.min.css');
$context->addJsFile('\static\libs\gallery\js\blueimp-gallery.min.js');
...
if (($type = Html::getRequestOrPost('datetype', false, HTML::TEXT)) !== false && in_array($type, array(
    'DateCliche',
    'DateUpload'
)) === true) {
...
$periods = $context->getDb()
    ->photos()
    ->select('Distinct Year(' . $_SESSION['photos_filters']['dateType'] . ') As Annee')
...
$photos = $context->getDb()
    ->photos()
    ->where('Year(' . $_SESSION['photos_filters']['dateType'] . ')', $_SESSION['photos_filters']['annee']);
...
<div id="blueimp-gallery" class="blueimp-gallery">
	...
	<a class="next" title="Suivante">›</a>
	<a class="close" title="Fermer">×</a>
	<a class="play-pause" title="Lecture"></a>
	<ol class="indicator"></ol>
</div>
<div>
	<a href="/photos/upload.php" class="btn btn-primary pull-right">
		<i class="icon-plus-sign icon-white"></i>&nbsp;Envoyer de nouvelles photos
	</a>
	<a class="btn btn-success pull-right" style="margin-right: 15px;" id="startSlideshow" onClick="javascript:blueimp.Gallery($('#gallery_photos').find('a'));">
		<i class="icon-picture icon-white"></i>&nbsp;Diaporama
	</a>
	<div id="gallery_photos" style="display: none;">
<?php
foreach ($photos as $p) {
    ?>
<a href="/photos/photo.jpg.php?id=<?=$p['id']; ?>"></a>
<?php
}
?>
</div>
	<form method="GET" class="form-inline">
		<fieldset>
			<select name="datetype" onChange="javascript:this.form.submit();">
				<option value="DateCliche" <?=($_SESSION['photos_filters']['dateType'] === 'DateCliche' ? ' selected="selected"' : ''); ?>>Date du
					cliché</option>
				<option value="DateUpload" <?=($_SESSION['photos_filters']['dateType'] === 'DateUpload' ? ' selected="selected"' : ''); ?>>Date d'envoi
					sur le site</option>
			</select>
			<select name="annee" onChange="javascript:this.form.submit();">
				<?php
    foreach ($periods as $p) {
        echo '<option value="' . $p['Annee'] . '"' . (intval($p['Annee']) === $_SESSION['photos_filters']['annee'] ? ' selected="selected"' : '') . '>' . $p['Annee'] . '</option>';
    }
    ?>
			</select>
			<select name="mois" onChange="javascript:this.form.submit();">
				...
				<?php
    for ($i = 1; $i <= 12; $i ++) {
        echo '<option value="' . $i . '"' . ($i === $_SESSION['photos_filters']['mois'] ? ' selected="selected"' : '') . '>' . Db_Sql::formatSqlDate('2000-' . $i . '-01 00:00:00', '%B') . '</option>';
    }
    ?>
			</select>
		</fieldset>
...
foreach ($photos as $p) {
    ...
    ?>
			<li class="span2">
				<div class="thumbnail">
					<a href="/photos/photo.php?id=<?=$p['id']; ?>">
						<img alt="<?=$p['Titre']; ?>" src="/photos/photo.jpg.php?id=<?=$p['id']; ?>&f=mini">
					</a>
				</div>
			</li>
			<?php
