...
if ($_SESSION['TOKEN'] === Html::getRequestOrPost('token', false, HTML::TEXT)) {
    ...
    if (is_array($message) === true && isset($message['discussions_id']) === true && Html::getPost('message[message]', false, Html::TEXT) !== false && ($d_id = Html::getPost('message[discussions_id]', false, Html::INTEGER)) !== false) {
        ...
        if ($m_id !== 0) {
            ...
            $lastMessage = $db->discussions_messages()
                ->where('discussions_id', $d_id)
                ->order("DateMessage desc")
                ->limit(1)
                ->fetch();

            $editable = (intval($lastMessage['id']) === $m_id && $lastMessage['personnes_id'] === $context->getUser()->getId()) || $context->getUser()->hasAuth('Gestion') === true;
...
<?php
            ...
            $res = $db->discussions_messages()->insert(array(
                'discussions_id' => $d_id,
                'DateMessage' => Db_Sql::getNowString(),
                'personnes_id' => $context->getUser()
                    ->getId(),
        ...
        $lastMessage = $db->discussions_messages()
            ->where('discussions_id', $id)
            ->order("DateMessage desc")
            ->limit(1)
            ->fetch();

        $editable = (intval($lastMessage['id']) === intval($m_id) && intval($lastMessage['personnes_id']) === intval($context->getUser()->getId())) || $context->getUser()->hasAuth('Gestion') === true;
...
function showForm($d, $m = null)
{
    ...
    Context::getInstance()->addJsFile('/static/libs/bootstrap-wysihtml5/dist/bootstrap-wysihtml5-0.0.2.js', 'G');
    Context::getInstance()->addJsFile('/static/libs/bootstrap-wysihtml5/src/locales/bootstrap-wysihtml5.fr-FR.js', 'G');
...
<form method="post" class="clearfix" action="/discussions/discussion.php?id=<?=$d['id']; ?>">
	...
	<?php
    Context::getInstance()->addJsInline("$('#messageEditArea" . ($m === null ? 0 : $m['id']) . "').wysihtml5({locale: 'fr-FR'});");
    ?>
</form>
<?php
}
				...
				<tr>
					<td class="message clearfix">
						...
						<div class="formMessageArea clearfix">
							...
							<?php
    Context::getInstance()->addJsInline("$('#messageEditArea0').wysihtml5({locale: 'fr-FR'});");
    ?>
						</div>
					</td>
				</tr>
...
<div class="discussion block">
    ...
    $lastMessage = $messages->order("DateMessage desc")
        ->limit(1)
        ->fetch();
    ...
    foreach ($messages->order("DateMessage desc")->limit($nbPerPage, ($_SESSION['discussion_page'] - 1) * $nbPerPage) as $m) {
        $m['id'] = intval($m['id']);
        $editable = (intval($lastMessage['id']) === intval($m['id']) && intval($m['personnes_id']) === intval($context->getUser()->getId())) || $context->getUser()->hasAuth('Gestion') === true;
