...
if ($_SESSION['TOKEN'] === Html::getRequestOrPost('token', false, HTML::TEXT)) {
    ...
    if (($new_coordonnees_id = Html::getRequestOrPost('new_coordonnees_id', false, HTML::INTEGER)) !== false) {
        $context->getDb()
            ->personnes_coordonnees()
            ->insert(array(
            'personnes_id' => $id,
            'coordonnees_id' => $new_coordonnees_id
        ));
    ...
    if ($coordEdit !== false) {
        ...
        $new_coordonnees_id = $context->getDb()
            ->coordonnees()
            ->insert($newAddr);
        $context->getDb()
            ->personnes_coordonnees()
...
$enfants = $context->getDb()
    ->personnes()
    ->select('*, Year(DateNaissance) As AnneeNaissance')
    ->where('IdParent1 = ? OR IdParent2 = ?', $membre['id'], $membre['id'])
    ->order('DateNaissance');
...
<form method="POST" action="?id=<?=$id; ?>">
    ...
    ?> class="btn">Annuler</a>
		...
		<h2>
			...
			<small><?=($membre['NomJF'] !== null ? '(' . $membre['NomJF'] . ') ' : ''); ?> <?=$context->getSql()->formatSqlDate($membre['DateNaissance'], '%d/%m/%Y') . ($membre['DateMort'] === null ? '' : ' - ' . $context->getSql()->formatSqlDate($membre['DateMort'], '%d/%m/%Y'));?></small>
		</h2>
	</div>
	<div class="row">
		<div class="span6">
    ...
    $conjoints = $context->getDb()
        ->personnes_liens_couple('IdPersonne1 = ? OR IdPersonne2 = ?', array(
        $membre['id'],
        $membre['id']
    ))
        ->select('*, IF(IdPersonne1 = ' . $membre['id'] . ', IdPersonne2, IdPersonne1) AS personnes_id')
        ->order('id');
    foreach ($conjoints as $c) {
        ?>
					<div class="alert alert-info">
						...
						<a href="/famille/membre.php?id=<?=$c->personnes['id']; ?>"><?=$c->personnes['Prenom'] . ' ' . $c->personnes['Nom'];?></a> <br />Date du mariage :
    ...
    ?>
					<div class="alert alert-success">
						<a href="javascript:setRelativePerson('Conjoint', '<?=$membre['id']; ?>', '<?=$_SESSION['NEXT_TOKEN']; ?>');">Ajouter un conjoint...</a>
					</div>
				</div>
    ...
    foreach ($conjoints as $c) {
        foreach ($context->getDb()
            ->personnes()
            ->where('(IdParent1 = ? AND IdParent2 IS NULL) OR (IdParent2 = ? AND IdParent1 IS NULL)', $c['personnes_id'], $c['personnes_id']) as $e) {
            ?>
					<div class="alert alert-success">
...
<div class="span6 pull-right">
    ...
    if ($id !== 0) {
        ...
        ?>
			<div class="alert alert-success">
				<?php
        $proches = $context->getDb()
            ->personnes()
            ->where('id IN (?, ?) OR IdParent1 IN (?, ?, ?) OR IdParent2 IN (?, ?, ?) OR id IN (SELECT IdPersonne1 FROM personnes_liens_couple WHERE IdPersonne2 = ?)OR id IN (SELECT IdPersonne2 FROM personnes_liens_couple WHERE IdPersonne1 = ?)', array(
            $membre['IdParent1'],
            $membre['IdParent2'],
            $membre['IdParent1'],
            $membre['IdParent2'],
        ...
        $coordsProches = $context->getDb()
            ->personnes_coordonnees('personnes_id', $proches)
            ->select('*, coordonnees.*')
            ->where('NOT coordonnees_id', $membre_coordonnees)
            ->group('coordonnees_id');
        ...
        ?>
				<fieldset>
			<h5>Rattacher à une nouvelle adresse</h5>
			<form method="POST" id="newAddrForm">
				<input type="hidden" name="token" value="<?=$_SESSION['NEXT_TOKEN']; ?>" />
				<fieldset>
					<textarea id="newCoordsAddr" name="coord[Adresse]" rows=2 placeholder="Adresse" required="required" class="input-xlarge"></textarea> <br />
					<input type="text" id="newCoordsZip" name="coord[CodePostal]" value="" placeholder="Code postal" class="input-small" />
					<input type="text" id="newCoordsCity" name="coord[Ville]" value="" placeholder="Ville" required="required" class="input-large" />
					<br />
					<input type="text" id="newCoordsCountry" name="coord[Pays]" value="France" placeholder="Pays" required="required" class="input-large" />
					<br />
					<input type="tel" id="newCoordsTel" name="coord[Tel]" value="" placeholder="Téléphone" class="input-medium" />
					<br />
					<input type="tel" id="newCoordsFax" name="coord[Fax]" value="" placeholder="Fax" class="input-medium" />
					<button type="button" class="btn btn-primary pull-right" onClick="javascript:addAdress('<?=$_SESSION['NEXT_TOKEN']; ?>', '<?=$membre['id']; ?>');">Ajouter</button>
				</fieldset>
			</form>
		</fieldset>
